...
Use SHA-256 hashing with canonical JSON serialization for request fingerprinting. The canonical representation is generated by serializing a list of request fields `[protocol, action, target, sorted_headers, body_hex]` using `json.dumps` with `separators=(",", ":")` and `sort_keys=True` to ensure determinism and prevent delimiter collisions.
...
- **Canonical ordering**: Sorting headers and using fixed JSON separators ensures identical requests produce identical fingerprints regardless of environment
...
- Headers order doesn't affect matching (sorted canonically)
...
- **Cons**: Headers dict order dependency would cause false mismatches, large memory footprint for storing full request data as keys
- **Reason for rejection**: Order-dependent matching breaks determinism; memory inefficient
